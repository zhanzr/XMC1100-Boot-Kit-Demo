...
/**************************************************************************/ /**
 Purpose:        Implementation of fast square root algorithm after Otto Peter's
                 (Otto Peter: Prozessor zieht Wurzel, c't 1990, Heft 1, page
 300-306)
...
static uint32_t sqrt_5(uint32_t x) {
  ...
  if (xroot < x)
    return xroot + 1; // rounding, necessary
...
/**************************************************************************/ /**
 ...
 Detailed description: The function is used to convert the ANATSEMON values to
 Kelvin.
...
uint32_t XMC1000_CalcTemperature(void) {
  ...
  /** Check the higher limit, if above use configANATSEMON_max as reverence
   * value */
  ...
  /* If the calculated temperature is in the expected range, add the correction
  ...
  return (calcTemperature +
          tse_corrdata); // TSE_CorrData[calcTemperature - MIN_TEMP_KELVIN]
}
...
/**************************************************************************/ /**
 ...
 Detailed description: The function is used to convert Kelvin temperatures to
 ANATSEMON values.
...
uint32_t XMC1000_CalcTSEVAR(uint32_t temperature) {
  ...
  /* Lookup for the TSE sensor value corresponding to the real temperature; if
   * nothing is found, use the user value. */
