...
void TestFunct(void) { printf("CPUID:%08X\n", SCB->CPUID); }
...
#define TEST_ADD_MUL_NUM 100000
  // If the muliplication takes similar cycles, it is a single cycle
  ...
  printf("%08X,%08X,%08X\n", (uint32_t)g_Thread1, (uint32_t)g_Thread2,
  ...
  //
  //		tmpTick = osKernelSysTick();
  //		while((tmpTick+osKernelSysTickMicroSec(2000)) >
  // osKernelSysTick())
  //		{
  //			__NOP();
  //			__WFI();
